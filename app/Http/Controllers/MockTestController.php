    ...
    public function index($id)
{
    ...
    $data['create_test'] = MockTest::withTrashed()
        ->where('volume_id', $id)
        ->paginate(10);

    session(['test_volume' => $id]);

    return view('admin.mocktest.index')->with($data);
}


    public function add(){
        $data['page'] = 'Add Tests';
        return view('admin.mocktest.add')->with($data);
    }
    public function edit($id){
        $data['page'] = 'Edit Tests';
        $data['edit'] =MockTest::withTrashed()->findOrFail($id);
        return view('admin.mocktest.edit')->with($data);
    }
    ...
    public function update(Request $request, $id)
{
 ...
 public function destroy($id)
    {
        $test = MockTest::withTrashed()->findOrFail($id);
...
{
    MockTest::withTrashed()->findOrFail($id)->forceDelete();
    ...
    {
        $batch = MockTest::withTrashed()->findOrFail($id);
        $batch->restore();
